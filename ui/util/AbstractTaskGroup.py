...
class AbstractTaskGroup:
    ...
    def add_task(self, task, prepend=""):
        if prepend != "":
            wrapper = TextWrapper(width=self.taskline_container.width-2-Settings.get('appearance.window_padding')*2-len(prepend), initial_indent="",subsequent_indent=" "*self.taskline_container.indent, drop_whitespace=False, term=term)
        else:
